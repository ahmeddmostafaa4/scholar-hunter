...
# The dropdown id-SUFFIXES are mostly the same on GUC and GIU. The full field
# NAME (with its ContentPlaceHolder prefix) differs between them, so we never
# hard-code it: `_field_name` reads it off the page by matching this suffix.
# The season dropdown is the one exception - GUC calls it "Dropdownlistseason",
# GIU calls it "ddl_season" - so we match either.
YEAR_SUFFIX = "stdYrLst"
SEASON_SUFFIX = "Dropdownlistseason|ddl_season"
...
class GucPortal:
    ...
    def _get(self, path: str) -> tuple[str, str]:
        """GET a page, priming the session and following the bot-check redirect.

        Returns (html, url).

        GIU bounces the first hit on a deep page through Default.aspx to Home
        while it sets up the session cookie, so we'd otherwise parse the Home
        page (no dropdowns) instead of the page we asked for. When we notice
        we've landed somewhere other than the page we requested, we retry once:
        the cookie is set now, so the second hit reaches the real page.
        """
        target = self.base_url + path
        requested_page = path.split("?")[0].rsplit("/", 1)[-1]
        response = self.session.get(target, timeout=self.timeout)
        response.raise_for_status()
        if requested_page and requested_page not in response.url:
            response = self.session.get(target, timeout=self.timeout)
            response.raise_for_status()
